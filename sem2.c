...
struct Farmacie* copiereElemente(struct Farmacie* vector, int nrFarmacii, int nrObiecteCopiate) {
...
struct Farmacie* copiazaFarmaciiMici(struct Farmacie* farmacii, int nrFarmacii, float pragSuprafata, int* nrFarmaciiMici) //transmis prin pointer
{
	*nrFarmaciiMici = 0;
	for (int i = 0; i < nrFarmacii; i++) {
...
void dezalocareVectorFarmacie(struct Farmacie** vector, int* nrFarmacii) //cu doua stelute pt a gestiona dangling pointers (adresa unde retin adresa vectorului)
{
	for (int i = 0; i < *nrFarmacii; i++) {
		free((*vector)[i].nume);
...
int main()
{
	//int v1[40]; ///alocat static, se afla pe stack ul functiei main (exemplul cu mersul la film)
	//int *v2=
	int nrFarmacii = 4;
	...
	struct Farmacie* farmaciiCopiate = copiereElemente(farmacii, nrFarmacii, nrFarmaciiCopiate);
	printf("\n\n");
	...
	dezalocareVectorFarmacie(&farmacii, &nrFarmacii);
	dezalocareVectorFarmacie(&farmaciiCopiate, &nrFarmaciiCopiate);
	dezalocareVectorFarmacie(&FarmaciiMici, &nrFarmaciiMici);
}